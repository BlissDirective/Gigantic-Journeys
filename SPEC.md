...
Version 1.0 · 2026-09-15 · Owner: BlissDirective (SparkForge Labs) · Maintainer: Coordinator (Claude Code)
...
Gigantic Journeys is a mobile game (iOS and Android, one Unity 6 codebase) in which the player scans a real place, a room or a tabletop build, becomes a near-photorealistic 1:12 avatar of themselves, and journeys through that place: reaching its summit, running its routes, finding its vistas. The environment is the content; the avatar is the piece; traversal is the product. Finished environments can be published to an opt-in global database where they are ranked and played by others.
...
- ARKit/ARCore camera poses and depth recorded alongside video. Live coverage heat-map; speed meter that turns amber when moving too fast; blur rejection with a gentle haptic; a "you missed this corner" hint before upload; a quality gate that rejects early and kindly ("Too dark here — turn on a lamp?").
...
- App Store privacy labels and the Play data safety form accurate to the frozen telemetry schema.
...
## 6. Performance and quality budgets

| Budget | Value | Source |
|---|---|---|
| Frame rate on the reference Android device (2023 mid-tier; exact model fixed by AUTH at the M0 checkpoint, candidate Samsung Galaxy A54 5G) | ≥ 30 fps p99 in play, Tier 0 and Tier 1 on | Bible §2, §13; Design Skills rule 27 |
| Frame rate on current iPhones | 60 fps target | Design Skills rule 27 |
...
| Motion-warping accuracy | hands within 0.05 A of the edge on 20 of 20 test edges | Bible §13 |

Every gameplay or UI PR attaches a Profiler screenshot from the reference device (Design Skills §4).
...
| **M3 Game loop and tabletop** | 7–10 | The Owner and three testers each explore five environments and want a sixth; Tier 1 holds 30 fps on the reference device. |
...
| **M5 Store readiness and IAP** | 12–16 | Both stores submitted after the Owner's explicit approval; 100 testers at 99 %+ crash-free. |
...
1. A new user on either platform scans a room or a Lego build in under 90 seconds of active capture.
...
5. The app holds ≥ 30 fps p99 on the reference Android device with Tier 0 and Tier 1 on, and ≥ 99 % crash-free sessions across 100 testers.
...
7. Both stores have approved the app.
8. No synthetic game object exists in the shipped build (§4).

## 10. Glossary

**A** one avatar height, the unit for every movement threshold · **Environment** a captured place with its derived package · **Summit / route / vista** the three goal types (§3.4) · **Tier 0 / 1 / 2** reactivity levels (§3.6, §5) · **Splat** the Gaussian-splat representation of a scan · **Scale multiplier** the per-environment factor applied to the default 1:12 · **Beat** a route segment's narrative role (introduce, develop, twist, resolve) · **Environment package** the published bundle (§3.7) · **Reference device** the 2023 mid-tier Android phone every budget is measured on.
