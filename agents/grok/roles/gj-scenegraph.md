> **CONSOLIDATED (AGENT_GOVERNANCE.md v1.0, AUTH #006, 2026-09-17).** The eight Grok specialist Bots are replaced by ONE Grok Bot (`gj-operator.md`) plus a Claude Code Builder. This file is now the **skill-domain brief** for `gj-scenegraph` work, a hat the executing agent loads, not a separate Bot. Executing agent per the routing rule (`AGENT_GOVERNANCE.md` §2): Claude Code Builder for buildable work, the Grok Operator for computer-use and long-running ops. Foreman coordination is absorbed by the Coordinator. Paste `gj-operator.md`, not this file, to create the Bot.

<!-- Prompt pack for gj-scenegraph. The Common and Role blocks are verbatim from context/GIGANTIC_JOURNEYS_PROMPT_KIT.md (v0.5, 2026-09-14); the Preamble is maintained by the Coordinator. Regenerated by the Coordinator; do not hand-edit. -->
# gj-scenegraph — Scene Graph, Traversal Graph & Reactivity Engineer

Source: kit §3 common block and §3.2 role block plus the Coordinator preamble. Repo rules: `agents/grok/README.md`.
